...
describe("Filter box testing", () => {
  ...
  it("All content should be render", async () => {
    ...
    render(<MultiSelectBox selectedValue={[]} selectedAttr={[]} {...selectProps} />);
    const selectBox = screen.getByTestId("multiselect-box")
  ...
  it("Check selected value", async () => {
    render(<MultiSelectBox selectedValue={[]} selectedAttr={[]} {...selectProps} />);
    const selectVlaues = ['Option1100', 'Option1200']
    ...
    await act(async () => 
      await user.click(within(optionsPopupEl).getByText(/Option1200/i))
    )
  ...
  it("Check clear all", async () => {
    render(<MultiSelectBox selectedAttr={[]} selectedValue={['Option1100', 'Option1200']} {...selectProps} />);
  ...
  it("Check select all", async () => {
    render(<MultiSelectBox selectedAttr={[]} selectedValue={[]} {...selectProps} />);
    // Open select box
    const selectInput = screen.getByRole("button")
    await act(async () => {
      await user.click(selectInput);
